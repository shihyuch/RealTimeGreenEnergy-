  ...
  */
  function getPopup(place) {
    ...
    var info = {
      ...
      sunrise: moment(place.sunrise).tz(timeZoneId).format('YYYY-MM-DD hh:mm:ss A'),
      sunset: moment(place.sunset).tz(timeZoneId).format('YYYY-MM-DD hh:mm:ss A'),
	  sun_power: (10*((moment(place.sunset) - moment(place.sunrise))/86400000)).toFixed(2).toString()+ ' kW',
	  ...
	  // Using Wind speed to calculate how much power by Wind
	  wind: (0.15*(place.wind * place.wind)).toFixed(2).toString() + ' kW',
	  // Using Temperature to calculate danger score measured by percentage
      danger: ((place.danger - 253.15)/273.15 * 300).toFixed(2).toString() + ' %',
	  // Using Humidity to calculate how much power by Water
      humidity: (0.0015*(place.humidity * place.humidity)).toFixed(2).toString() + ' kW'
    }
